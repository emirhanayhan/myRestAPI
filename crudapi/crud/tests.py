...
class testUser(APITestCase):
    ...
    def test_post_user(self):
        data = {'name': 'yeniali', 'mail': 'yeniali@gmail.com', 'status': 0}
    ...
    def test_update_user(self):
        pk = 35
        data2 = {'name': 'ali2', 'mail': 'ali2@gmail.com', 'status': 0}
        response = self.client.put(reverse('user-detail', kwargs={'pk': pk}), data=json.dumps(data2),
...
class testUserTrip(APITestCase):
    ...
    def test_post_trip(self):
        data = {'userId_id': 33, 'totalDistance': 55, 'beginningTime': '07:53pm'}
        response = self.client.post(self.url, data=data)
        result = response.json()
        self.assertEqual(response.status_code, 201)
    ...
    def test_update_trip(self):
        pk = 1
        data2 = {'userId_id': 33, 'totalDistance': 65, 'beginningTime': '05:25am'}
        response = self.client.put(reverse('trip-detail', kwargs={'pk': pk}), data=json.dumps(data2),
                                   content_type='application/json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
